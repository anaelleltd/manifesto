...
\title{Polkadot Fellowship Manifesto \\ {\smaller \textbf{Draft 6}}}
...
\begin{itemize}
 ...
 \item the Polkadot networking stack;
 ...
 \item core technologies needed to enable the existence of the Polkadot Network and its Native feature set;
 \item user-interface code required to carry out the technical work (designing, programming, debugging) required to maintain Polkadot (Main) Network, including operational upgrades; and
...
This leniency, such as it is, should exist only for the initial bootstrapping phase. By the first continuation evaluation, the full expectations of the member's rank should be met for approval by voters.
...
\item Promotion:
  ...
  \end{enumerate}
\item If the promotion is approved, they attain Membership and their associated rank increments to a value of one, unless stated otherwise.
\end{enumerate}
...
\item Promotion:
  ...
  \end{enumerate}
\item If the promotion is approved, their associated rank is incremented by one, unless stated otherwise.
\end{enumerate}
...
Once over a period of time (three months for members below the rank of Fellow, six months for non-Master members who are Fellows or beyond) are automatically demoted unless their rank is re-approved by their peers. This upvote is similar to a evaluation and they should submit clear and concise argument showing they are retaining the qualities expected of a member of their rank and have been contributing substantially for the period of time they have been on standard allowance.
...
\subsubsection{Requirements for I Dan}\label{requirements-1}
...
\subsubsection{Requirements for II Dan}\label{requirements-2}
...
The individual should by now be thinking in a very much in a security-conscious, game-theoretic way, understanding that the systems whose design they are increasingly a part of must function adequately even with a modest minority of malicious users. Recent reviews of their design work should betray no argument that the individual is either failing to consider this or unable to think in such a way.
...
\subsubsection{Requirements for III Dan}\label{requirements-3}

\begin{itemize}
\item \textbf{Promotion should involve an in-person evaluation during which superiors may invite the individual to answer specific questions concerning their contributions.}
...
\subsubsection{Requirements for IV Dan}\label{requirements-4}
...
\subsubsection{Requirements for V Dan}\label{requirements-5}
...
\subsubsection{Requirements for VI Dan}\label{requirements-6}
...
Note: This is the first rank for which pre-specified discussion points become a crucial part of the evaluation and where those voting on the promotion must make a qualitative assessment on the outcome of the discussion in comparison to the precedent of prior promotions.
...
\subsubsection{Requirements for VII Dan}\label{requirements-7}
...
\subsubsection{Requirements for VIII Dan}\label{requirements-8}
...
\subsubsection{Requirements for IX Dan}\label{requirements-9}
...
Members are free to place themselves on a passive allowance if they believe they are unlikely to contribute substantially within any given month. (At challenge or evaluation time, members who do not may find the lack of productivity difficult to explain.) Regardless, members must keep their knowledge and skills up to date.
...
At least once per year the members of the Fellowship should gather. This should be paid for by the treasury. Lower evaluations (I-III Dan) should happen here (though I and II Dan may happen remotely).

There should be additional meetings exclusively for Fellows and (when there are practical numbers) Masters. Information on these meetings should (for security purposes) be kept on a need-to-know basis. Disregarding this is a breach of Fellowship rules. Upper evaluations (IV-VI Dan) should happen here.
...
\begin{itemize}
  \item Introducing shadowing a pre-existing Fellow as a requirement for Fellow evaluation.
  ...
  \item In general, evaluations should be conducted in-person. From III Dan evaluation and above, they \textbf{MUST} be in-person unless environmental conditions make it impossible (e.g.~pandemic). If the individual wishes to remain anonymous, a mask or other facial-covering may be worn.
